...
export const LoanService = {
    ...
    return await AppDataSource.transaction(
      async (transactionalEntityManager) => {
        ...
        for (const copyBookId of loanRequest.bookIds) {
          ...
          if (copyBook.status === CopyBookStatus.BORROWED) {
            throw new BadRequestException(
              "Copy book is already borrowed: " + copyBook.barcode,
            );
          }
          ...
          if (!copyBook.book) {
            throw new NotFoundException(
              "Associated book not found for this copy book",
            );
          }
          const bookId = copyBook.book.bookId;

          const countAvailableCopy = await transactionalEntityManager.count(
            CopyBook,
            {
              where: {
                book: { bookId: bookId },
        ...
        const fullLoanData = await transactionalEntityManager.findOne(Loan, {
          ...
          relations: {
            user: false,
            loanDetails: { copyBook: { book: false } },
          },
        });
        return fullLoanData;
      },
      ...
      async (transactionalEntityManager) => {
        ...
        if (copyBooks.length !== barcodes.length) {
          ...
          const missingBarcodes = barcodes.filter(
            (b) => !foundBarcodes.includes(b),
          );
          throw new NotFoundException(
            `Copy books not found: ${missingBarcodes.join(", ")}`,
          );
        }
        ...
        for (const detail of loanDetails) {
          ...
          const linkedCopyBook = copyBooks.find(
            (cb) => cb.copyBookId === detail.copyBook.copyBookId,
          );
          if (linkedCopyBook) {
            linkedCopyBook.status = CopyBookStatus.AVAILABLE;
            await transactionalEntityManager.save(linkedCopyBook);
            const bookId =
              linkedCopyBook.book.bookId || (linkedCopyBook as any).bookId;
            if (bookId) {
        ...
        for (const loanId of affectedLoanIds) {
          const remainingBorrowingBooks =
            await transactionalEntityManager.count(LoanDetail, {
              where: {
                loan: { loanId },
          ...
          if (remainingBorrowingBooks === 0) {
            const hasOverdueBook = await transactionalEntityManager.count(
              LoanDetail,
              {
                where: {
                  loan: { loanId },
            ...
            const finalStatus =
              hasOverdueBook > 0 ? LoanStatus.OVERDUE : LoanStatus.RETURNED;
  ...
  getLoanDetailByLoanId: async (
    ...
    const isAdminOrLibrarian = user.roles.some(
      (r: any) =>
        r.roleName === RoleName.ADMIN || r.roleName === RoleName.LIBRARIAN,
    );
      ...
      loanDetails: loan.loanDetails.map((ld) => {
        return {
          ...
          returnDate: ld.returnDate
            ? ld.returnDate.toISOString().slice(0, 10)
            : "",
          status: ld.status,
        };
      }),
  ...
  getAllLoanDetails: async () => {
    ...
    const data = loanDetails.map((ld) => ({
      ...
      borrowDate: ld.loan?.borrowDate
        ? new Date(ld.loan.borrowDate).toISOString().slice(0, 10)
        : "",
      dueDate: ld.loan?.dueDate
        ? new Date(ld.loan.dueDate).toISOString().slice(0, 10)
        : "",
      loanDetailId: ld.loanDetailId,
      returnDate: ld.returnDate
        ? new Date(ld.returnDate).toISOString().slice(0, 10)
        : "",
      status: ld.status,
      bookId: ld.copyBook?.book?.bookId || "",
      bookName: ld.copyBook?.book?.title || "",
  ...
  getLoanDetails: async (
    ...
    const list: LoanDetailResponse[] = loanDetails.map((item) => {
      ...
      const borrowedRemain = Math.floor(
  ...
  async getLoanHomeData(userId: string): Promise<LoanHomeResponse> {
    ...
    const recentLoanDetail =
      await loanDetailRepository.getRecentLoanDetailByUserId(userId);
    let progress = 0;
    let recentLoanResponse = null;
    if (recentLoanDetail) {
      const borrowDate = recentLoanDetail.loan.borrowDate;
      const dueDate = recentLoanDetail.loan.dueDate;
      const endDate = recentLoanDetail.returnDate
        ? recentLoanDetail.returnDate
        : new Date();
